...
public class AnsjHandler extends AbstractHandler {
	...
	public void handleRequest(HttpServerExchange exchange) throws Exception {
		...
		switch(path) {
		case "fenci":
			list = FenciUtil.fenci(text, NumberUtil.parseEnum(getParam(exchange, "method"), Method.TO));
			break;
		case "keywords":
			int num = NumberUtil.parseInt(getParam(exchange, "num"), 10);
			list = FenciUtil.keywords(getParam(exchange, "title"), text, num, NumberUtil.parseEnum(getParam(exchange, "method"), Method.TO));
			break;
		case "summary":
			boolean red = NumberUtil.parseBoolean(getParam(exchange, "red"), true);
			string = FenciUtil.summary(getParam(exchange, "title"), text, red);
			break;
		case "jianfan":
			boolean fan = NumberUtil.parseBoolean(getParam(exchange, "fan"), true);
